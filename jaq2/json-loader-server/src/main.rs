...
pub use json_loader_server::query_engine::execute_query;

const JSON_DIR: &str = "/home/ubuntu/jaq2/jaq/loadjson";
const DB_PATH: &str = "/home/ubuntu/jaq2/jaq/json-loader-server/database.db";
...
pub fn create_app(
    state: Arc<json_loader_server::AppState>,
    cors: CorsLayer,
) -> Router {
    Router::new()
        ...
        .route("/source-files/:filename", delete(delete_source_file_by_path))
...
async fn load_json_files(
    State(state): State<Arc<json_loader_server::AppState>>,
...
async fn get_files(State(state): State<Arc<json_loader_server::AppState>>) -> Result<Json<Vec<String>>, StatusCode> {
    let db = state.db.lock().unwrap();
...
fn rows_to_objects(columns: &[String], rows: &[Vec<serde_json::Value>]) -> Vec<serde_json::Map<String, serde_json::Value>> {
    rows.iter()
        .map(|row| {
    ...
    match execute_query(&db, &request.query) {
        Ok(result) => {
            ...
            let query_plan = if request.query.contains("join") || request.query.contains("JOIN") {
                format!("Table scan + Join operations (scanned ~{} rows)", total_db_rows)
            } else {
            ...
            match format {
                "json" => {
                    ...
                    response.insert("data".to_string(), serde_json::to_value(rows_to_objects(&result.columns, &result.rows)).unwrap_or_default());
                    response.insert("performance".to_string(), serde_json::to_value(performance).unwrap_or_default());
                    Ok(([(header::CONTENT_TYPE, "application/json")], 
                        serde_json::to_string(&response).unwrap_or_default()).into_response())
                }
                "jsonl" => {
            ...
            let notes: HashMap<String, QueryNote> = match db.prepare(
                ...
                Ok(mut stmt) => {
                    match stmt.query_map(params![&query_hash], |row| {
                        ...
                        Ok(rows) => rows.filter_map(|r| r.ok()).map(|n| (n.row_hash.clone(), n)).collect(),
                        Err(_) => HashMap::new(),
                    }
                }
                Err(_) => HashMap::new(),
    ...
    match execute_query(&db, &request.query) {
        Ok(result) => {
            let filename = request.filename.unwrap_or_else(|| {
                format!("jaq_export_{}", chrono::Local::now().format("%Y%m%d_%H%M%S"))
            });
        ...
        Err(e) => {
            Ok((StatusCode::BAD_REQUEST, format!("Export error: {}", e)).into_response())
        }
    }
}

fn export_to_csv(result: &query_engine::QueryResult, filename: &str) -> Result<Response, StatusCode> {
    ...
    // Write headers
    wtr.write_record(&result.columns).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    
    // Write rows
    for row in &result.rows {
        let record: Vec<String> = row.iter().map(|cell| {
            match cell {
        ...
        wtr.write_record(&record).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    }
    ...
    Ok((
        [
            ...
            (header::CONTENT_DISPOSITION, &format!("attachment; filename=\"{}.csv\"", filename)),
        ],
        data
    ).into_response())
}

fn export_to_xlsx(_result: &query_engine::QueryResult, _filename: &str) -> Result<Response, StatusCode> {
    // XLSX export temporarily disabled - requires libclang for bindgen
    Err(StatusCode::NOT_IMPLEMENTED)
}

fn export_to_parquet(result: &query_engine::QueryResult, filename: &str) -> Result<Response, StatusCode> {
...
fn export_to_jsonl(result: &query_engine::QueryResult, filename: &str) -> Result<Response, StatusCode> {
    ...
    Ok((
        [
            ...
            (header::CONTENT_DISPOSITION, &format!("attachment; filename=\"{}.jsonl\"", filename)),
        ],
        data
    ).into_response())
}
    ...
    match val {
        ...
        Val::Num(n) => {
            match n {
                ...
                Num::Int(i) => serde_json::Value::Number(
        ...
        Val::Arr(arr) => {
            serde_json::Value::Array(arr.iter().map(jaq_val_to_json).collect())
        }
        Val::Obj(obj) => {
...
async fn save_query_history(
    ...
    match db.execute(
        "INSERT INTO query_history (query_json, query_type, source_tab, result_count) 
         VALUES (?1, ?2, ?3, ?4)",
        params![&request.query, &query_type, &request.source_tab, &request.result_count],
    ) {
        Ok(_) => {
            // Get the last inserted id
            let id: i64 = db.last_insert_rowid();
...
async fn save_note(
    ...
    match db.execute(
        ...
        params![&request.query_hash, full_query, &request.row_hash, &request.note_text],
    ) {
        Ok(_) => {
...
async fn delete_note(
    ...
    match db.execute(
        "DELETE FROM query_notes WHERE id = ?1",
        params![id],
    ) {
        Ok(rows_affected) => {
            if rows_affected > 0 {
...
async fn list_source_files(
    ...
    let files: Vec<SourceFileInfo> = stmt.query_map([], |row| {
        Ok(SourceFileInfo {
...
async fn clear_all_source_files(
    ...
    // Get count before deletion
    let count: i64 = db.query_row("SELECT COUNT(*) FROM json_objects", [], |row| row.get(0))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
fn extract_field_type(value: &serde_json::Value) -> String {
    match value {
        ...
        serde_json::Value::Number(n) => {
            if n.is_i64() { "integer".to_string() } else { "number".to_string() }
        }
...
fn collect_nested_fields(obj: &serde_json::Map<String, serde_json::Value>, prefix: &str, fields: &mut HashSet<String>) {
...
async fn get_schema(
    ...
    let mut stmt = match db.prepare(
        "SELECT json_data FROM json_objects WHERE source_file = ?1 LIMIT 100"
    ) {
        Ok(s) => s,
        Err(_) => return Err(StatusCode::INTERNAL_SERVER_ERROR),
    };
...
async fn analyze_joins(
    ...
    let mut stmt = match db.prepare(
        "SELECT DISTINCT source_file FROM json_objects"
    ) {
        Ok(s) => s,
        Err(_) => return Err(StatusCode::INTERNAL_SERVER_ERROR),
    };
    
    let source_files: Vec<String> = match stmt.query_map([], |row| {
        row.get::<_, String>(0)
    }) {
        Ok(rows) => rows.filter_map(|r| r.ok()).collect(),
        Err(_) => return Err(StatusCode::INTERNAL_SERVER_ERROR),
    ...
    for (left_field, left_values) in &left_fields {
        for (right_field, right_values) in &right_fields {
            ...
            if is_likely_join_field(left_field) && is_likely_join_field(right_field) {
                ...
                if matches > 0 {
                    let match_percentage = (matches as f64 / left_values.len().min(right_values.len()) as f64) * 100.0;
...
fn get_sample_data(db: &Connection, source_file: &str, limit: usize) -> Result<Vec<serde_json::Value>, rusqlite::Error> {
    let mut stmt = db.prepare(
        "SELECT json_data FROM json_objects WHERE source_file = ?1 LIMIT ?2"
    )?;
    
    let rows: Vec<serde_json::Value> = stmt.query_map(params![source_file, limit], |row| {
        let json_str: String = row.get(0)?;
        let value: serde_json::Value = serde_json::from_str(&json_str)
            .map_err(|e| rusqlite::Error::InvalidParameterName(e.to_string()))?;
        Ok(value)
            ...
            serde_json::Value::Array(arr) => {
                ...
                for item in arr {
                    if let serde_json::Value::Object(nested) = item {
                        extract_string_fields_from_obj(nested, &format!("{}[]", field_path), fields);
                    } else if let serde_json::Value::String(s) = item {
                        if !s.is_empty() {
                            fields.entry(format!("{}[]", field_path)).or_default().insert(s.clone());
                        }
                    }
                }
            }
...
fn is_likely_join_field(field_name: &str) -> bool {
    ...
    let join_keywords = ["id", "sku", "code", "mpn", "gtin", "barcode", "handle", "variant"];
...
fn init_database() -> Result<Connection> {
    ...
    if !column_exists {
        conn.execute(
            "ALTER TABLE query_notes ADD COLUMN full_query TEXT",
            [],
        )
...
async fn ai_generate_dsl(Json(request): Json<AiQueryRequest>) -> Result<Response, StatusCode> {
    ...
    let files: Vec<&str> = request.prompt
        .lines()
    ...
    let match_type = if prompt_lower.contains("barcode") || prompt_lower.contains("gtin") || prompt_lower.contains("ean") || prompt_lower.contains("upc") {
        "barcode"
    ...
    for file in &files {
        let alias = file.trim_end_matches(".json").replace("_", "").replace("-", "");
        
        // Try to load sample data from the database
        let sample_data = if let Ok(conn) = rusqlite::Connection::open(DB_PATH) {
            if let Ok(mut stmt) = conn.prepare("SELECT json_data FROM json_objects WHERE source_file = ?1 LIMIT 1") {
    ...
    if analyses.len() >= 2 {
        ...
        let mut joins = vec![];
        let mut select = vec![
            serde_json::json!({"expr": format!("{}.*", primary_alias), "alias": "primary"})
        ];
        ...
        for (i, analysis) in analyses.iter().skip(1).enumerate() {
            ...
            let join_field = match field_type {
                "barcode" => {
                    analysis.barcode_field.clone()
                        .or_else(|| analysis.mpn_field.clone())
                        .unwrap_or_else(|| "mpn".to_string())
                }
                "sku" => {
                    analysis.sku_field.clone()
                        .or_else(|| analysis.mpn_field.clone())
                        .unwrap_or_else(|| if analysis.source_file.contains("shopify") { "variants[].sku".to_string() } else { "default_code".to_string() })
                }
                _ => {
                    analysis.sku_field.clone()
                        .or_else(|| analysis.barcode_field.clone())
                        .or_else(|| analysis.mpn_field.clone())
                        .unwrap_or_else(|| if analysis.source_file.contains("shopify") { "variants[].sku".to_string() } else { "mpn".to_string() })
                }
            };
...
async fn main() -> Result<()> {
    ...
    let state = Arc::new(json_loader_server::AppState::new(conn));
