...
pub struct SelectField {
    /// Field expression (e.g., "gmc.title", "shopify.status")
    #[serde(default)]
...
impl<'a> QueryEngine<'a> {
    ...
    fn execute_with_context(
        ...
        let where_op = query.where_op.as_deref().unwrap_or("and");
        let mut residual_where = query.r#where.clone();
        ...
        let (primary_alias, primary_data) = if let Some(ref subquery_name) = query.from_subquery {
            ...
            let (source_pushdown, new_residual_where) =
                self.build_source_pushdown(&alias, query.r#where.as_deref(), where_op);
            residual_where = new_residual_where;

            if let Some(ref explode_config) = from.explode_with_context {
                let data = self.load_source(from, source_pushdown.as_ref())?;
                let exploded = self.explode_with_context(data, &alias, explode_config)?;
                ...
                if let Some(conditions) = residual_where.as_deref() {
                    joined_data = self.apply_filters(joined_data, conditions, where_op)?;
                }

                let total_count = joined_data.len();

                // Step 4: Flatten if requested
                if query.flatten {
                    joined_data = self.flatten_rows(joined_data)?;
                }

                // Step 5: Apply distinct if requested
                if query.distinct {
                    joined_data = self.apply_distinct(joined_data)?;
                }

                // Step 6: Apply group_by and aggregation if requested
                let (columns, mut rows) = if let Some(ref group_by) = query.group_by {
                    ...
                    self.select_fields(joined_data, &query.select, query.limit)?
                };

                // Step 7: Apply order_by sorting
                if let Some(ref order_by) = query.order_by {
                    rows = self.apply_order_by(rows, &columns, order_by)?;
                }
    ...
    fn load_source(&self, source: &Source, pushdown: Option<&SqlPushdown>) -> Result<Vec<Value>> {
        let source_file = &source.source_file;
        let mut sql = String::from("SELECT json_data FROM json_objects WHERE source_file = ?");
        let mut sql_params = vec![SqlValue::Text(source_file.clone())];

        if let Some(push) = pushdown {
            sql.push(' ');
            sql.push_str(&push.sql);
            sql_params.extend(push.params.clone());
        }
    ...
    fn build_source_pushdown(
        &self,
        source_alias: &str,
        conditions: Option<&[Condition]>,
        where_op: &str,
    ) -> (Option<SqlPushdown>, Option<Vec<Condition>>) {
        ...
        let is_or = where_op.eq_ignore_ascii_case("or");
        let mut pushed_fragments = Vec::new();
        let mut pushed_params = Vec::new();
        let mut residual = Vec::new();

        for cond in conditions {
            if let Some((fragment, mut params)) = self.condition_to_sql(source_alias, cond) {
                pushed_fragments.push(fragment);
                pushed_params.append(&mut params);
            } else {
                residual.push(cond.clone());
            }
        }

        // For OR semantics, pushdown is only safe if all conditions are pushdown-compatible.
        if is_or && !residual.is_empty() {
            return (None, Some(conditions.to_vec()));
        }

        let pushdown = if pushed_fragments.is_empty() {
            None
        } else {
            let joiner = if is_or { " OR " } else { " AND " };
            Some(SqlPushdown {
                sql: format!("AND ({})", pushed_fragments.join(joiner)),
    ...
    fn condition_to_sql(
        ...
        let json_path = self.pushdown_json_path(source_alias, &cond.field)?;
        let expr = format!("json_extract(json_data, '{}')", json_path);
        let op = cond.op.to_lowercase();

        match op.as_str() {
            "is_null" => Some((format!("{expr} IS NULL"), vec![])),
            "is_not_null" | "exists" => Some((format!("{expr} IS NOT NULL"), vec![])),
            "=" | "==" => {
                let value = cond.value.as_ref()?;
                if value.is_null() {
                    Some((format!("{expr} IS NULL"), vec![]))
                } else {
                    let param = self.json_scalar_to_sql(value)?;
                    Some((format!("{expr} = ?"), vec![param]))
                }
            }
            "!=" | "<>" => {
                let value = cond.value.as_ref()?;
                if value.is_null() {
                    Some((format!("{expr} IS NOT NULL"), vec![]))
                } else {
                    let param = self.json_scalar_to_sql(value)?;
                    Some((format!("{expr} != ?"), vec![param]))
                }
            ...
            ">" | ">=" | "<" | "<=" => {
                let value = cond.value.as_ref()?;
                let param = self.json_scalar_to_sql(value)?;
                Some((format!("{expr} {} ?", op.to_uppercase()), vec![param]))
            }
            _ => None,
        }
    }

    fn pushdown_json_path(&self, source_alias: &str, field: &str) -> Option<String> {
        let normalized = field.trim();
        let field_path = if let Ok((alias, path)) = self.parse_field_ref(normalized) {
    ...
    fn json_scalar_to_sql(&self, value: &Value) -> Option<SqlValue> {
        match value {
    ...
    fn explode_with_context(
        ...
        if let Some(ref conditions) = config.r#where {
            result = result
                .into_iter()
                .filter(|row| self.matches_conditions(row, conditions, "and"))
                .collect();
        }
    ...
    fn apply_join_with_context(
        ...
        let (right_alias, right_data) = if let Some(ref source) = join.source {
            ...
            let data = self.load_source(source, None)?;
        ...
        for left_row in &left_data {
            ...
            // If skip_nulls is enabled and left key is null/empty, skip the join lookup
    ...
    fn apply_filters(
        ...
        Ok(data
            .into_iter()
            .filter(|row| self.matches_conditions(row, conditions, op))
            .collect())
    }
    ...
    fn matches_conditions(
        ...
        match op {
            "or" => conditions
                .iter()
                .any(|cond| self.matches_condition(row, cond)),
            _ => conditions
                .iter()
                .all(|cond| self.matches_condition(row, cond)), // default "and"
        }
    }
    ...
    fn matches_condition(&self, row: &HashMap<String, Value>, cond: &Condition) -> bool {
        let (alias, field) = match self.parse_field_ref(&cond.field) {
            Ok(r) => r,
            Err(_) => return false,
        };
    ...
    fn apply_group_by(
        ...
        if let Some(having_conditions) = having {
            ...
            rows.retain(|row| {
                ...
                self.matches_conditions(&row_map, having_conditions, having_op)
            });
        }
    ...
    fn select_fields(
        ...
        for row in data.iter().take(limit.unwrap_or(usize::MAX)) {
            ...
            for field in regular_fields.iter() {
                let value = self.evaluate_select_field(row, field)?;
    ...
    fn evaluate_select_field(
        ...
        if let Some(result) = self.evaluate_arithmetic_expression(row, expr) {
            return Ok(result);
        }
    ...
    fn evaluate_arithmetic_expression(
        ...
        if let Some(idx) = expr.find(" * ") {
            ...
            let left_val = self.evaluate_expression_operand(row, left)?;
            let right_val = self.evaluate_expression_operand(row, right)?;
    ...
    /// Evaluate an operand that could be a field reference or a number
    fn evaluate_expression_operand(
        ...
        if operand.starts_with('(') && operand.ends_with(')') {
            ...
            return self.evaluate_arithmetic_expression(row, inner);
        }
...
mod tests {
    ...
    fn test_date_function_evaluate() {
        ...
        // Create a mock QueryEngine (we can't easily create one without a DB connection,
        // but we can test the DateFunction directly)

        // Test extract_value_by_path
        let value = DateFunction::extract_value_by_path(&row, "te.inDate", unsafe {
            // This is a hack for testing - we need a QueryEngine reference
            std::mem::zeroed()
        });

        // Since we can't easily test with a real QueryEngine, let's just verify the row structure
