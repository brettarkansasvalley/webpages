...
class TipDistributionTouchApp(tk.Tk):
    def __init__(self):
        ...
        self._scroll_job_id = None # Para gestionar el desplazamiento continuo
    ...
    def _configure_styles(self):
        ...
        s.configure("Scroll.TButton", font=("Helvetica", 16, "bold"), padding=5)
        
        # --- NUEVO: Estilo de Scrollbar sin flechas ---
        s.layout("Arrowless.TScrollbar", 
            [('Vertical.Scrollbar.trough', {'children': [
                ('Vertical.Scrollbar.thumb', {'expand': '1', 'sticky': 'ns'})
            ], 'sticky': 'ns'})]
        )
        s.configure("Arrowless.TScrollbar", width=SCROLLBAR_WIDTH)
    ...
    def create_workers_tab(self):
        ...
        # Configurar la cuadrícula para el marco de los trabajadores
        workers_display_frame.grid_rowconfigure(0, weight=1)
        workers_display_frame.grid_columnconfigure(0, weight=1)

        # Crear el lienzo y el marco desplazable
        canvas = tk.Canvas(workers_display_frame, highlightthickness=0)
        ...
        # --- Panel de Control de Desplazamiento Unificado ---
        controls_frame = ttk.Frame(workers_display_frame, width=SCROLLBAR_WIDTH)
        ...
        # Vincular todo al lienzo
        canvas.configure(yscrollcommand=scrollbar.set)
        scrollbar.configure(command=canvas.yview)

        up_button.bind('<ButtonPress-1>', lambda event, w=canvas, d=-1: self.start_scroll(w, d))
        up_button.bind('<ButtonRelease-1>', self.stop_scroll)

        down_button.bind('<ButtonPress-1>', lambda event, w=canvas, d=1: self.start_scroll(w, d))
        down_button.bind('<ButtonRelease-1>', self.stop_scroll)

        # Colocar los controles en la cuadrícula del panel
    ...
    def populate_cashbox_tab(self):
        """Populate the cashbox tab with data based on the selected button's query."""
        ...
        # Define query and parameters based on the selected drawer/button
        if drawer == "Office":
            # Query for non-bartender settlements
            query = 'SELECT id, timestamp, worker_name, reason, amount FROM cashbox_ledger WHERE reason != ? ORDER BY timestamp DESC'
            params = ('Bartender cash settlement',)
            balance_query = 'SELECT SUM(amount) FROM cashbox_ledger WHERE reason != ?'
            balance_params = ('Bartender cash settlement',)
        else:
            # Query for bartender settlements for a specific bar (AM, West Wing, Sunset)
            query = 'SELECT id, timestamp, worker_name, reason, amount FROM cashbox_ledger WHERE reason = ? AND cash_drawer = ? ORDER BY timestamp DESC'
            params = ('Bartender cash settlement', drawer)
            balance_query = 'SELECT SUM(amount) FROM cashbox_ledger WHERE reason = ? AND cash_drawer = ?'
            balance_params = ('Bartender cash settlement', drawer)
            
        # Execute the query to populate the treeview
        ...
        # Calculate and display the balance for the specific query
        balance_result = cursor.execute(balance_query, balance_params).fetchone()
        balance = balance_result[0] if balance_result and balance_result[0] is not None else 0.0
    ...
    def create_touch_payouts_display(self):
        """Create the touch payouts display with the unified scroll control panel."""
        ...
        # Contenedor principal que usará grid
        content_container = ttk.Frame(self.payouts_display_frame)
        ...
        # Panel de Control de Desplazamiento Unificado
        controls_frame = ttk.Frame(content_container, width=SCROLLBAR_WIDTH)
        controls_frame.grid(row=0, column=1, sticky='ns', padx=(5, 0))
        controls_frame.grid_propagate(False)
    ...
    def create_report_treeview(self, parent, columns, headings):
        """Create a report treeview with a unified, touch-friendly scroll control panel."""
        tree_frame = ttk.Frame(parent)
        tree_frame.pack(fill=tk.BOTH, expand=True)

        # Configurar la cuadrícula: 1 fila, 2 columnas (lista, panel de control)
        tree_frame.grid_rowconfigure(0, weight=1)
        tree_frame.grid_columnconfigure(0, weight=1)

        # Crear la lista (Treeview)
        ...
        # --- Panel de Control de Desplazamiento Unificado ---
        controls_frame = ttk.Frame(tree_frame, width=SCROLLBAR_WIDTH)
        controls_frame.grid(row=0, column=1, sticky='ns', padx=(5, 0))
        controls_frame.grid_propagate(False)
        ...
        # Vincular todo a la lista principal
        tree.configure(yscrollcommand=scrollbar.set)
        scrollbar.configure(command=tree.yview)
        
        up_button.bind('<ButtonPress-1>', lambda event, w=tree, d=-1: self.start_scroll(w, d))
        up_button.bind('<ButtonRelease-1>', self.stop_scroll)
        
        down_button.bind('<ButtonPress-1>', lambda event, w=tree, d=1: self.start_scroll(w, d))
        down_button.bind('<ButtonRelease-1>', self.stop_scroll)
    ...
    def start_scroll(self, widget, direction):
        """Inicia el desplazamiento continuo de un widget."""
        widget.yview_scroll(direction, 'units')
        # Repite esta función cada 60 milisegundos
        self._scroll_job_id = self.after(60, lambda: self.start_scroll(widget, direction))
